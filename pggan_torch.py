...
from fastestimator.trace.io import ModelSaver, RestoreWizard, Traceability
...
class StopTrace(Trace):
    def __init__(self, stop_epoch):
        super().__init__(inputs=None, outputs=None, mode="train")
        self.stop_epoch = stop_epoch

    def on_epoch_begin(self, state):
        if self.system.epoch_idx == self.stop_epoch:
            sys.exit("Stop by stop epoch at epoch {}".format(self.stop_epoch))
...
def get_estimator(stop_epoch,
    ...
    traces = [
        ...
        ImageSaving(
            ...
            save_dir=save_dir),
        RestoreWizard(directory="save2"),
        StopTrace(stop_epoch=stop_epoch)
        Traceability(save_path="report2")
    ]
...
if __name__ == "__main__":
    est = get_estimator(stop_epoch=int(sys.argv[1]))
    est.fit("exp")
